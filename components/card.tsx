...
export  function Card({shoe}: {shoe: Shoe}) {
    ...
    return (
           <Link onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}  href={`/product/${shoe._id}`} className="relative w-full h-60 sm:h-72 lg:h-96 shadow-xl flex flex-col overflow-hidden origin-center hover:scale-105 transition-all duration-200">
                    <motion.div animate={hover?{scale:1.7,y:72,opacity:0.6}:{scale:1,y:0,opacity:1}} transition={{duration:0.2,ease:"easeIn"}} className="relative sm:h-3/5 h-1/2 w-full overflow-hidden">
                      <Image src={shoe.imageUrl ?? '/assets/img/placeholder.png'} alt={shoe.name} fill className='object-cover object-center' />
                    </motion.div>
